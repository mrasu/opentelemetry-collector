...
func configureMetricOptions(t *testing.T, test metricsTestCase, initializeChs []chan bool, scrapeMetricsChs []chan int, closeChs []chan bool) []ScraperControllerOption {
	...
	for i := 0; i < test.scrapers; i++ {
		...
		scp, err := NewScraperFromComponentType(component.MustNewType("scraper"), tsm.scrape, scraperOptions...)
...
func TestSingleScrapePerInterval(t *testing.T) {
	...
	scp, err := NewScraperFromComponentType(component.MustNewType("scaper"), tsm.scrape)
	assert.NoError(t, err)
...
func TestScrapeControllerStartsOnInit(t *testing.T) {
	...
	scp, err := NewScraperFromComponentType(component.MustNewType("scraper"), tsm.scrape)
...
func TestScrapeControllerInitialDelay(t *testing.T) {
	...
	scp, err := NewScraperFromComponentType(component.MustNewType("timed"), func(context.Context) (pmetric.Metrics, error) {
		elapsed <- time.Now()
